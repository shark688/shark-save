...
- Python 3.10+
- Node.js 20+
- 通过 `imageio-ffmpeg` 内置 `ffmpeg`；如系统已安装则优先使用系统版本

`ffmpeg` 对 B 站等平台尤其重要。这类平台经常返回“视频流”和“音频流”分离的格式。SharkSave 现在会随后端依赖安装 `imageio-ffmpeg`，优先使用系统 `ffmpeg`，否则把内置二进制路径传给 `yt-dlp` 完成合并。
...
返回服务状态、当前服务端是否检测到 `ffmpeg`，以及 `ffmpeg` 来源。
